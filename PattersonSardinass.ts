...
function howToCode(code: string[]): { isCode: boolean, combination: any, temp: any }{
    let divisors: string[][] = multipleArrayVersion(code)
    let temp: string[] = []
    for(let i = 0; i < divisors.length; i++){
        let value = divisors[i]
        temp = substractArray(code, value)
        if(isCode(temp)) return { isCode: true, combination: value, temp: temp }
        temp = []
    }
...
let code = isCode("11111,11101,100,101111,00010,00011,001100,0100".split(","))
